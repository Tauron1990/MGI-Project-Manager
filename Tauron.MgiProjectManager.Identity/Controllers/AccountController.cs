...
using Tauron.MgiProjectManager.Identity.Models;
using Tauron.MgiProjectManager.Identity.Resources;

namespace Tauron.MgiProjectManager.Identity.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> UpdateUser(string id, [FromBody] UserEditViewModel user)
        {
            ...
            if (!string.IsNullOrWhiteSpace(user.Id) && id != user.Id)
                return BadRequest(IdentRes.AccountController_UpdateUser_IdMismatch);
            ...
            if (User.GetUserId() == id)
            {
                if (string.IsNullOrWhiteSpace(user.CurrentPassword))
                {
                    if (isPasswordChanged)
                        AddError(IdentRes.AccountController_UpdateUser_PasswordChanged, "Password");

                    if (isUserNameChanged)
                        AddError(IdentRes.AccountController_UpdateUser_UserChanged, "Username");
                }
                else if (isPasswordChanged || isUserNameChanged)
                {
                    if (!await _accountManager.CheckPasswordAsync(appUser, user.CurrentPassword))
                        AddError(IdentRes.AccountController_UpdateUser_PasswordCheck);
                }
            }
        ...
        public async Task<IActionResult> DeleteUser(string id)
        {
            ...
            if (!await _accountManager.TestCanDeleteUserAsync(id))
                return BadRequest(IdentRes.AccountController_DeleteUser_TestFailed);
            ...
            if (!result.Succeeded)
                throw new Exception(IdentRes.AccountController_DeleteUser_Error + string.Join(", ", result.Errors));
        ...
        public async Task<IActionResult> UnblockUser(string id)
        {
            ...
            var result = await _accountManager.UpdateUserAsync(appUser);
            if (!result.Succeeded)
                throw new Exception(IdentRes.AccountController_UnblockUser_Error + string.Join(", ", result.Errors));
        ...
        public async Task<IActionResult> UserPreferences([FromBody] string data)
        {
            ...
            var result = await _accountManager.UpdateUserAsync(appUser);
            if (!result.Succeeded)
                throw new Exception(IdentRes.AccountController_UserPreferences_Error + string.Join(", ", result.Errors));
        ...
        public async Task<IActionResult> UpdateRole(string id, [FromBody] RoleViewModel role)
        {
            ...
            if (!string.IsNullOrWhiteSpace(role.Id) && id != role.Id)
                return BadRequest(IdentRes.AccountController_UpdateRole_ConflictigId);
        ...
        public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            if (!await _accountManager.TestCanDeleteRoleAsync(id))
                return BadRequest(IdentRes.AccountController_DeleteRole_Test);
            ...
            if (!result.Succeeded)
                throw new Exception(IdentRes.AccountController_DeleteRole_Error + string.Join(", ", result.Errors));
