...
using Tauron.Application.Pipes.Threading;
...
namespace Tauron.Application.Pipes
{
        ...
        where TWrite : class
    {
        ...
        public void PushMessage(TWrite message)
        {
            lock (_connections)
            {
                foreach (var client in _connections) client.PushMessage(message);
            }
        }
        ...
        public void Stop()
        {
            ...
            lock (_connections)
            {
                foreach (var client in _connections.ToArray()) client.Close();
            }
        ...
        private async Task ListenSync()
        {
            _isRunning = true;
            while (_shouldKeepRunning)
                await WaitForConnection(_pipeName);
            _isRunning = false;
        }

        private Task WaitForConnection(string pipeName)
        {
        ...
        private Task ClientOnConnected(NamedPipeConnection<TRead, TWrite> connection)
        {
            if (ClientConnected != null)
                ClientConnected(connection);
        }

        private Task ClientOnReceiveMessage(NamedPipeConnection<TRead, TWrite> connection, TRead message)
        {
            if (ClientMessage != null)
                ClientMessage(connection, message);
        }

        private Task ClientOnDisconnected(NamedPipeConnection<TRead, TWrite> connection)
        {
            if (connection == null)
                return;
        ...
        /// </summary>
        private Task ConnectionOnError(NamedPipeConnection<TRead, TWrite> connection, Exception exception)
        {
            OnError(exception);
        }
