...
namespace Tauron.Application.ProjectManager.ApplicationServer.BussinesLayer
{
    ...
    public sealed class TimeCalculationCalculateTimeRule : IOBusinessRuleBase<CalculateTimeInput, CalculateTimeOutput>
    {
        ...
        public override CalculateTimeOutput ActionImpl([NotNull] CalculateTimeInput input)
        {
            ...
            var setupTime = TimeSpan.FromMinutes(SettingsManager.Default.SetupTime);

            // ReSharper disable PossibleInvalidOperationException

            var iterationTime   = TimeSpan.FromMinutes(SettingsManager.Default.IterationTime * (int) input.Iterations);
        ...
        private int CalculateEffectiveAmount(int amount, int iterations) => amount * iterations;

        private TimeSpan FindPerfectRuntime(List<JobRunEntity> entrys, int amount) => TimeSpan.FromMinutes(entrys.Average(e => e.NormaizedTime.TotalMinutes) * (amount / 1000d));
        ...
        private ImmutableSortedDictionary<double, List<JobRunEntity>> Sort(IEnumerable<JobRunEntity> entry, double speed)
        {
            ...
            foreach (var clusterEntry in entry)
            {
                ...
                {
                    if (!key.AlmostEqual(clusterSpeed, SettingsManager.Default.PefectDifference)) continue;
        ...
        private Dictionary<PrecisionMode, List<JobRunEntity>> AggregateEntitys(CalculateTimeInput input)
        {
            ...
            var settingsManager      = SettingsManager.Default;
            ...
            var expire        = settingsManager.EntityExpire;
            var tolerance     = settingsManager.NearCornerDifference;
            var amount        = input.Amount.Value;


            using (RepositoryFactory.Enter())
            {
                var repo = RepositoryFactory.GetRepository<IJobRunRepository>();
                ...
                {
                    if (entry.Speed.AlmostEqual(speed, settingsManager.PefectDifference))
                    ...
                    else if (entry.Speed.AlmostEqual(speed, settingsManager.NearCornerDifference))
                        if (IsAmoutMismatch(entry.Amount, amount))
        ...
        {
            var tolerance = SettingsManager.Default.AmoutMismatch;
